...
def test_code_repair_env_rejects_finish_before_visible_success_side_effects():
    ...
    try:
        ...
        env.close()
def test_code_repair_env_rejects_finish_after_failed_visible_test_with_refresh_guidance():
    env = create_code_repair_env()

    try:
        env.reset()
        step_env(env, VIEW_ALL_ACTION)
        step_env(env, WRONG_SOLUTION_ACTION)
        _obs, _rewards, dones, infos = step_env(env, RUN_VISIBLE_ACTION)
        ...
        assert dones == [False]
        assert rewards[0] < 0.0
        assert "view_problem(section=all)" in obs[0]
        assert infos[0]["code_repair_next_required_action"] == "view_problem"
        assert infos[0]["code_repair_protocol_expected_action"] == "view_problem"
        assert infos[0]["code_repair_protocol_accepted"] is False
        assert infos[0]["code_repair_protocol_side_effect_applied"] is False
...
def test_code_repair_env_oversized_replace_is_invalid_without_side_effects_or_order_credit():
    ...
    try:
        ...
        assert infos[0]["code_repair_protocol_side_effect_applied"] is False
        assert infos[0]["code_repair_required_action_match"] is False
        assert infos[0]["code_repair_edit_count"] == 0
...
def test_code_repair_env_full_test_in_loop_can_unlock_finish_when_enabled():
    from agent_system.environments.env_package.code_repair.envs import CodeRepairVectorEnv

    task = create_code_repair_task()
    env = CodeRepairVectorEnv(
        tasks=[task],
        env_num=1,
        group_n=1,
        seed=0,
    ...
    try:
        ...
        obs, rewards, dones, infos = step_env(
            env,
            "<function=run_tests><parameter=suite>full</parameter></function>",
        )

        assert dones == [False]
        assert "Full tests: 4/4 passed" in obs[0]
        assert infos[0]["code_repair_protocol_state_after"] == "can_finish"
        assert infos[0]["code_repair_next_required_action"] == "finish"
        assert infos[0]["code_repair_visible_passed_current_code"] is True

        _obs, _rewards, dones, infos = step_env(env, FINISH_READY_ACTION)
